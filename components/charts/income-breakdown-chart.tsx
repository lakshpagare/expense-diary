...
import type { IncomeCategoryBreakdownItem } from "@/lib/data";

export function IncomeBreakdownChart({
  breakdown,
  currency = "INR",
}: {
  breakdown: IncomeCategoryBreakdownItem[];
  currency?: string;
}) {
  const router = useRouter();

  const goToCategory = (category: string) => {
    router.push(`/income?category=${encodeURIComponent(category)}`);
  };

  return (
    <Card>
      <CardContent>
        <h3 className="text-base font-semibold text-foreground">Income by Category</h3>
