...
function otpEmailHtml(name: string, otp: string, heading: string, bodyLine: string): string {
  ...
  <div style="font-family: -apple-system, Segoe UI, Roboto, sans-serif; max-width: 480px; margin: 0 auto; padding: 32px 24px; color: #0f172a;">
    <div style="display:flex; align-items:center; gap:8px; margin-bottom: 24px;">
      ...
      <span style="font-size:16px; font-weight:600;">Expenses and Incomes Trackers</span>
    </div>
...
const FROM_ADDRESS = process.env.EMAIL_FROM || "Expenses and Incomes Trackers <onboarding@resend.dev>";
...
export async function sendVerificationOTP(to: string, name: string, otp: string) {
  ...
  await resend.emails.send({
    from: FROM_ADDRESS,
    to,
    subject: "Verify your email - Expenses and Incomes Trackers",
    html: otpEmailHtml(
      name,
      otp,
...
export async function sendLoginOTP(to: string, name: string, otp: string) {
  ...
  await resend.emails.send({
    from: FROM_ADDRESS,
    to,
    subject: "Your login code - Expenses and Incomes Trackers",
    html: otpEmailHtml(
      name,
      otp,
      "Confirm it's you",
